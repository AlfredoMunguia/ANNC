...
Matrix<T,1,Dynamic>* BdyTerm<T>::biasGradient(int l){  
  ...
  for(int k=0; k < _nTrain; k++){        
    *_gradb[l] +=   (*_va[k][L+1])[1] * _vB[k][l]->block(0,1,_architecture[l+1],1).transpose();    //Second column
  }
...
Matrix<T,Dynamic,Dynamic>* BdyTerm<T>::weightGradient(int l){  
  ...
  for(int k=0; k < _nTrain; k++){    
    *_gradW[l] += _va[k][l]->transpose() * (*_va[k][L+1])[1] * _vB[k][l]->block(0,1,_architecture[l+1],1).transpose();
  }
